...
const Contact = () => {
    ...
    const rotatePlate = (e) => {
        ...
        x = x - xlength;
        movePalette.current.style.transform = `rotateX(${-((y - (ylength / 2)) / (ylength)) * 40}deg) rotateY(${(-(xlength / 2 - x) / xlength) * 40}deg)`
    }
